...
const AlbumWrapper = ({ id }) => {
  ...
  return (
    <>
      <h2 class="discographyPage_title">{albumList.data[0]?.artist.name}</h2>
      ...
      <div class="prevAndNext_buttons">
        {!!albumList.prev && (
          <button onClick={() => getAlbums(albumList.prev)}>PREV</button>
        )}

        {!!albumList.next && (
          <button onClick={() => getAlbums(albumList.next)}>NEXT</button>
        )}
      </div>

      <div>{albumList.data == 0 && <NoArtistData />}</div>
    </>
  );
};
